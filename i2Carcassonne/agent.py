...
def determine_closest_resource_tile(resource_tiles, unit, player, purpose):
...
def determine_colony_location(cities, game_state, unit, resource_tiles):
    ...
    for colony_loc_option in possible_colony_tiles:
        option_dist = colony_loc_option.pos.distance_to(unit.pos)
...
def agent(observation, configuration):
    ...
    logging.info("starting turn. number of citytiles = " + str(cityTile_count))
    ...
    for unit in player.units:
        ...
        if unit_cities[unit.id] not in player.cities.keys() and unit_assignments[unit.id] != "Colonize":
        ...
        if unit.is_worker() and unit.can_act():
            ...
            if unit.id in unit_assignments.keys():
                ...
                if assignment == "Feed": #one issue with this function is that there is no dropoff mode where the unit will empty its whole inventory. rather it will drop of for one turn and then harvest more once it has any space
                    ...
                    if unit.get_cargo_space_left() < 5:
                        ...
                        tile_for_harvest = determine_closest_resource_tile(resource_tiles, unit, player, "fuel")
                    ...
                    if unit.get_cargo_space_left() == 0:
                        ...
                        tile_for_harvest = determine_closest_resource_tile(resource_tiles, unit, player, "build")
                        if tile_for_harvest != None:
                            if tile_for_harvest.pos == unit.pos:
                                action_selection = None
                            else:
                    ...
                    if colony_location != None:
                        ...
                        actions.append(annotate.text(colony_location.pos.x, colony_location.pos.y, "colonize!" + str(unit.id), 16))
                        ...
                        if unit.get_cargo_space_left() == 0:
                            ...
                            tile_for_harvest = determine_closest_resource_tile(resource_tiles, colony_location, player, "build")
            ...
            #             build_location = determine_city_expansion_location(cities, game_state, unit)
            ...
            #             else:
            #                 move_dir, move_tile = get_directions(unit, game_state, build_location, True)
            #                 movement_is_picked = handle_unit_movement(unit, game_state, move_dir, move_tile, movement_is_picked)
